...
function Get-TeamExact {
    <#
      Busca um time por nome. Estratégia (validada em homologação):
        - Usar GET /api/teams/search?query=X (LIKE server-side)
        - Filtrar client-side com Trim() + comparação case-insensitive,
          para absorver espaços/casing diferentes no cadastro do team.
        Em alguns builds do Grafana o parâmetro ?name= não retorna match,
        então usamos ?query= como primário.
    #>
    param([string]$Name)

    $r = Invoke-Grafana -Method GET -Path "/api/teams/search?query=$([uri]::EscapeDataString($Name))"
    if ($r.StatusCode -ne 200) { return @{ Found=$false; Error=$r } }

    $teams = @()
    if ($r.Body -and ($r.Body.PSObject.Properties.Name -contains 'teams') -and $r.Body.teams) {
        $teams = @($r.Body.teams)
    }
    Write-Log "Team search '$Name' retornou $($teams.Count) candidato(s): $(@($teams | ForEach-Object { "[$($_.id)='$($_.name)']" }) -join ', ')" 'DEBUG'

    $target = $Name.Trim().ToLowerInvariant()
    $match = $teams | Where-Object {
        $_ -and $_.PSObject.Properties.Name -contains 'name' -and
        $_.name -and $_.name.Trim().ToLowerInvariant() -eq $target
