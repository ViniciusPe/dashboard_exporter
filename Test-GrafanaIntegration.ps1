...
function Get-TeamExact {
    <#
      Busca um time por nome. Estratégia:
        1) GET /api/teams/search?name=X  (match exato case-sensitive no servidor)
        2) Se vazio: GET /api/teams/search?query=X (LIKE) + filtro case-insensitive no cliente
      Isso cobre diferenças de caixa/espaços no nome cadastrado.
    #>
    param([string]$Name)
    ...
    $teams = @()
    if ($r.Body -and ($r.Body.PSObject.Properties.Name -contains 'teams')) { $teams = @($r.Body.teams) }
    $match = $teams | Where-Object { $_.name -eq $Name } | Select-Object -First 1

    if (-not $match) {
        # Fallback: LIKE com 'query', case-insensitive client-side
        $r2 = Invoke-Grafana -Method GET -Path "/api/teams/search?query=$([uri]::EscapeDataString($Name))"
        if ($r2.StatusCode -eq 200 -and $r2.Body -and ($r2.Body.PSObject.Properties.Name -contains 'teams')) {
            $match = @($r2.Body.teams) | Where-Object { $_.name -ieq $Name } | Select-Object -First 1
            if ($match) { Write-Log "Team '$Name' encontrado via fallback query= (match case-insensitive)" 'WARN' }
        }
    }
...
function Get-UserByEmail {
    <#
      IMPORTANTE: /api/users/lookup exige role GRAFANA SERVER ADMIN.
      Service Account em uma Org (mesmo com role Admin da Org) recebe 403.
      Estratégia recomendada para integração:
        Usar /api/org/users?query={email} - funciona com Org Admin.
        Retorna array de OrgUser com { orgId, userId, email, login, name, role }.
      Retorno padronizado: PSObject com .StatusCode e .Body (com .id ou .userId).
    #>
    param([string]$Email)

    $r = Invoke-Grafana -Method GET -Path "/api/org/users?query=$([uri]::EscapeDataString($Email))"
    if ($r.StatusCode -ne 200) { return $r }

    $users = @($r.Body)
    $match = $users | Where-Object { ($_.email -ieq $Email) -or ($_.login -ieq $Email) } | Select-Object -First 1
    if ($match) {
        # Normaliza: expõe .id (igual ao /users/lookup) para o resto do script
        $match | Add-Member -NotePropertyName 'id' -NotePropertyValue $match.userId -Force
        return [pscustomobject]@{ StatusCode = 200; Body = $match; RawError = $null }
    }
    return [pscustomobject]@{ StatusCode = 404; Body = $null; RawError = "Usuário '$Email' não encontrado em /api/org/users" }
}
...
# ============================================================
# T10 - Token inválido: 401 esperado (endpoint que SEMPRE exige auth)
# ============================================================
Write-Log '--- T10: Token inválido deve retornar 401 (em /api/user) ---' 'STEP'
# Obs.: /api/org pode permitir anônimo em algumas configs.
# /api/user (current signed-in user) sempre exige token válido.
$bad = Invoke-Grafana -Method GET -Path '/api/user' -OverrideToken 'glsa_invalid_token_for_test'
if ($bad.StatusCode -eq 401) {
    Add-Result 'T10' '401 com token inválido' 'OK' ''
} elseif ($bad.StatusCode -eq 200) {
    Add-Result 'T10' '401 com token inválido' 'FAIL' 'Endpoint aceitou anônimo - verificar auth.anonymous no Grafana'
} else {
