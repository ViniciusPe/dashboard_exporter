...
```
1. Scheduler dispara (a cada 180 min por padrão, configurável via DQS_EVALUATION_INTERVAL_MINUTES)
2. Service busca lista de todos dashboards via Grafana API (com paginação automática, sem limite)
...
│        │ ⚠ Melhorias: • 17 painéis sem descrição                        │
│        │              • 5 queries sem $__rate_interval                   │
│        │              • Sem links para runbook ou documentação           │
│        │ 🕒 Última avaliação: 31/08/2026 14:30 UTC                       │
│        │ ⏭️ Próxima avaliação: 31/08/2026 17:30 UTC                      │
└─────────────────────────────────────────────────────────────────────────┘
```

- Badge circular com a nota e cor da faixa
- Até 4 melhorias prioritárias (maior severidade primeiro)
- Horário da última avaliação e previsão da próxima
...
recommendations:
  panel_descriptions:
    text: "Adicione descrição nos painéis ({count} sem descrição)"
    action: "Edite cada painel > Panel options > Description"
  
  
  dashboard_title_convention:
    text: "Nome não segue padrão 'Área - Tecnologia - Objetivo' (mínimo 3 segmentos)"
...
- Atualizado a cada ciclo do scheduler (a cada 180 minutos por padrão)
- Identificado por campo `"dqs_managed": true` no JSON do panel
- Exibe horário da última avaliação e previsão da próxima
- Se deletado pelo usuário, será re-injetado no próximo ciclo
- Painéis existentes são deslocados para acomodar (gridPos.y += 4)
...
|---|---|---|---|
| **Título do dashboard** | 30% | Alta | Deve seguir o padrão: `Área - Tecnologia - Objetivo` com mínimo 3 segmentos separados por " - ". Ex: `NoSQL - MongoDB - Overview Metrics` ou `O11y - Integração e Conectividade - Azure Event Hub - Overview` |
...
| **Descrição dos painéis** | 25% | Alta | Todo painel deve possuir campo `description` preenchido, explicando o que é exibido e sua relevância |
...
- Título "Dashboard MongoDB" → falta padrão Área - Tecnologia - Objetivo (apenas 2 segmentos)
...
**Nota sobre painéis HTML/Text:** Painéis do tipo `text` que não possuem datasource nem targets (puramente informativos) são automaticamente ignorados na avaliação, não penalizando o score.
...
| **Uso de variáveis de template** | 25% | Alta | Queries Prometheus devem utilizar variáveis de template (`$var`) para filtros, permitindo interatividade e reuso. **Nota:** Regra só se aplica a queries Prometheus (com campo `expr`). Dashboards com outros datasources (Azure Monitor, Infinity, etc.) recebem score 100 nesta regra |
| **Sem métricas soltas** | 20% | Alta | Toda query Prometheus deve usar pelo menos uma função de agregação ou transformação (`sum`, `avg`, `rate`, `max`, `histogram_quantile`, etc.). Métrica bruta exposta sem função é penalizada. **Nota:** Só avalia queries Prometheus |
| **legendFormat definido** | 20% | Média | Toda query Prometheus deve ter `legendFormat` preenchido com labels relevantes. Legenda vazia ou `__auto` penaliza. **Nota:** Só avalia queries Prometheus |
| **Sem queries duplicadas** | 10% | Baixa | Não devem existir queries Prometheus idênticas (mesma `expr`) em painéis diferentes do mesmo dashboard |
...
> **Nota:** Datasource com UID fixo (hardcoded) **não é penalizado** pois os ambientes possuem UIDs padronizados.
...
- `?tag=<tag-name>` — Filtra apenas dashboards que possuem a tag especificada
...
```json
{
  ...
  "evaluated_at_ms": 1722695400000,
  "next_evaluation_ms": 1722706200000,
  "ranking": [
    {
    ...
    },
    {
      ...
      "score": 72.3,
      "label": "Bom",
      "top_issues": ["17 painéis sem descrição", "Sem links para runbook"],
      "evaluated_at": "2026-08-03T14:28:00Z"
    }
  ]
}
  ...
  "dimensions": [
    {
      ...
      "rules": [
        ...
        {
          "id": "panel_descriptions",
          "passed": false,
          "score": 0,
          "recommendation": "Adicione descrição nos 17 painéis sem descrição"
        }
      ]
    }
  ],
...
*Documento sujeito a revisão. Aguardando feedback do time para consolidação das regras e aprovação formal.*
